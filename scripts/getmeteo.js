...
const renderForecast = (forecast, indexVaros, varosPeldanynev) => {
  ...
  articleArray[
    indexVaros
  ].lastChild.textContent = `A hőmérséket: ${forecast.main.temp.toFixed(
    1
  )} Celsius`;
  addP = document.createElement("p");
  articleArray[indexVaros].append(addP);
  articleArray[
    indexVaros
  ].lastChild.textContent = `Felhőzet: ${forecast.weather[0].description}`;
  addP = document.createElement("img");
  articleArray[indexVaros].append(addP);
  articleArray[
    indexVaros
  ].lastChild.src = `https://openweathermap.org/img/wn/${forecast.weather[0].icon}@2x.png`;
  addP = document.createElement("p");
  articleArray[indexVaros].append(addP);
  articleArray[
    indexVaros
  ].lastChild.textContent = `Szélsebesség: ${forecast.wind.speed.toFixed(
    1
  )} m/s (${Math.round(forecast.wind.speed) * 3.6} km/h)`;
  addP = document.createElement("p");
  articleArray[indexVaros].append(addP);
  articleArray[indexVaros].lastChild.textContent = `Széllökés: ${
    forecast.wind.gust
  } m/s   (${Math.round(forecast.wind.gust) * 3.6} km/h)`;
  addP = document.createElement("p");
  articleArray[indexVaros].append(addP);
  articleArray[
    indexVaros
  ].lastChild.textContent = `Szélirány fokban: ${forecast.wind.deg}`;
  addP = document.createElement("span");
  articleArray[indexVaros].lastChild.append(addP);
  articleArray[indexVaros].children[7].lastChild.style.transform = `rotate(${
    forecast.wind.deg + 45
  }deg)`;
};
